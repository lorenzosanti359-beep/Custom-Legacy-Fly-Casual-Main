    ...
    public partial class AggressorAiPlayer : GenericAiPlayer
    {
        ...
        protected override void PerformActionFromList(List<ActionsList.GenericAction> actionsList)
        {
            ...
            foreach (var action in availableActionsList)
            {
                ...
                //Do not perform red action if this is not rotate arc action or barrel roll
                // (Barrel Roll: eccezione aggiunta perché la valutazione AI ora tiene conto
                // esplicitamente di ostacoli, navi, bordo tavolo e bullseye — vedi
                // NavigationSubSystem.TryBarrelRollPossibilities. Le altre azioni rosse
                // restano escluse: la logica generale di valutazione per azioni rosse
                // resta un obiettivo a lungo termine, non affrontato qui.)
                if (action.IsRed && !(action is ActionsList.RotateArcAction) && !(action is ActionsList.BarrelRollAction)) priority = int.MinValue;
