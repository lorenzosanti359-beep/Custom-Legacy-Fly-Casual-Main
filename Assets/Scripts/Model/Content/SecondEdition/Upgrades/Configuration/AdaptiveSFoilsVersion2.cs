...
{
    public class AdaptiveSFoilsVersion2Closed : GenericDualUpgrade
    {
        public AdaptiveSFoilsVersion2Closed() : base()
        {
            IsHidden = true;
            NameCanonical = "adaptivesfoilsVersion2-sideb";

            UpgradeInfo = new UpgradeCardInfo(
                "Adaptive S-Foils (Closed) Version2",
                ...
                abilityType: typeof(Abilities.SecondEdition.AdaptiveSFoilsVersion2ClosedAbility)
            );

            IsSecondSide = true;
            AnotherSide = typeof(AdaptiveSFoilsVersion2ClosedOpen);
        }
    }

    public class AdaptiveSFoilsVersion2ClosedOpen : GenericDualUpgrade
    {
        public AdaptiveSFoilsVersion2ClosedOpen() : base()
        {
            UpgradeInfo = new UpgradeCardInfo(
                "Adaptive S-Foils (Open) Version2",
                UpgradeType.Configuration,
                cost: 0,
                restriction: new ShipRestriction(typeof(Ship.SecondEdition.ASF01BWingMark2.ASF01BWingMark2)),
                abilityType: typeof(Abilities.SecondEdition.AdaptiveSFoilsVersion2OpenAbility)
            );

            AnotherSide = typeof(AdaptiveSFoilsVersion2Closed);
            NameCanonical = "adaptivesfoilsVersion2";
        }
    }
}
...
{
    public abstract class AdaptiveSFoilsVersion2CommonAbility : GenericAbility
    {
    ...
    public class AdaptiveSFoilsVersion2ClosedAbility : AdaptiveSFoilsVersion2CommonAbility
    {
        public override void ActivateAbility()
        {
            ...
            HostShip.BeforeTokenIsAssigned += CheckConditions;
        ...
        private void CheckConditions(GenericShip ship, GenericToken token)
        {
            ...
            (
                !ship.IsStrained
                && (token is StressToken || token is WeaponsDisabledToken)
            )
            {
                //TokenType = token.GetType();
                RegisterAbilityTrigger(TriggerTypes.OnBeforeTokenIsAssigned, RegisterTrigger);
            }
        }
        private void RegisterTrigger(object sender, EventArgs e)
        {
            AskToUseAbility(
               HostUpgrade.UpgradeInfo.Name,
               AlwaysUseByDefault,
               AskToReplaceToken,
               descriptionLong: "Do you want to assign an strain token instead?",
               imageHolder: HostUpgrade
           );
        }
    ...
    public class AdaptiveSFoilsVersion2OpenAbility : AdaptiveSFoilsCommonAbility
    {
